...
class SupabaseEnrichmentCache:
    """Cache for product enrichment data using Supabase as a backend store."""
    ...
    def generate_key(self, metadata: Dict[str, Any]) -> str:
        """
        Generate a unique cache key from product metadata.
        
        Args:
            metadata: Product metadata dictionary
            
        Returns:
            SHA256 hash of brand, name, and product type
        """
    ...
    def get(self, external_id: str) -> Optional[Dict[str, Any]]:
        """
        Retrieve cached product data from Supabase.
        ...
        try:
            ...
            product = {
                ...
                "name": data.get("name"),
                "product_type": None,  # Not in current schema
                "product_description": None,  # Not in current schema
                "enrichment": {
    ...
    def set(self, external_id: str, product_data: Dict[str, Any]) -> None:
        """
        Cache product data in Supabase.
        
        Args:
            external_id: Unique product identifier
            product_data: Product data dictionary to cache
        """
        try:
